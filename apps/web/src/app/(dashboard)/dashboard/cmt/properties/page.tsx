  ...
  return (
    <div className="space-y-6">
      ...
      {showTenantModal && selectedUnit && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Assign Tenant to {selectedUnit.name}</h2>
            <div className="space-y-2 max-h-96 overflow-y-auto mb-4">
              {selectedUnit.tenantId && (
                <button onClick={handleRemoveTenant} disabled={assigningTenant} className="w-full text-left px-4 py-3 rounded border border-red-300 bg-red-50 text-red-700 hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium mb-2">
                  {assigningTenant ? 'Removing...' : '✕ Remove Current Tenant'}
                </button>
              )}
              {availableTenants.length === 0 ? (
                ...
                availableTenants.map((tenant) => (
                  <button key={tenant.id} onClick={() => handleAssignTenant(tenant.id)} disabled={assigningTenant} className={`w-full text-left px-4 py-3 rounded border transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm ${selectedUnit.tenantId === tenant.id ? 'border-brand-blue bg-blue-50 text-brand-blue font-medium' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}>
                    {tenant.firstName} {tenant.lastName}
                    <div className="text-xs text-gray-500 mt-1">{tenant.user?.email}</div>
                  </button>
                ))
            ...
            <button onClick={() => { setShowTenantModal(false); setSelectedUnit(null); }} className="w-full px-4 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium">Close</button>
          </div>
        </div>
      )}
      ...
      {showLandlordModal && selectedUnit && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Assign Landlord to {selectedUnit.name}</h2>
            <div className="space-y-2 max-h-96 overflow-y-auto mb-4">
              {selectedUnit.landlordId && (
            ...
            <button onClick={() => { setShowLandlordModal(false); setSelectedUnit(null); }} className="w-full px-4 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-50 font-medium">Close</button>
          </div>
        </div>
      )}
  );
